...
/// Returns the color in the diagonally opposite yard.
/// Two-player games must always use these pairs for equal track spacing.
LudoColor ludoOppositeColor(LudoColor color) => switch (color) {
...
// Each color's path: 52 shared squares, 5 home-lane squares, then the
// center home. Positions 52-56 remain visible; position 57 is finished.
const ludoFinishPosition = 57;

bool canAdvanceLudoPiece(int position, int diceValue) {
  if (position < 0 || diceValue < 1 || diceValue > 6) return false;
  return position + diceValue <= ludoFinishPosition;
}

Offset ludoStackOffset(int index, int count, double cellSize) {
  if (count <= 1) return Offset.zero;
  final spread = cellSize * 0.20;
  if (count == 2) {
    return Offset(index == 0 ? -spread : spread, 0);
  }
...
class LudoPiece {
  ...
  int position; // -1 = base, 0-51 = track, 52-56 = lane, 57 = home
  bool isFinished;
...
class _LudoGameScreenState extends State<LudoGameScreen>
  ...
  void _initGame() {
    ...
    if (_numPlayers == 1) {
      // Two-player matches always use diagonally opposite yards so both
      // players travel the same relative distance around the board.
      final aiColor = ludoOppositeColor(_humanColor);
      final colors = [_humanColor, aiColor];
      for (int i = 0; i < 2; i++) {
        _players.add(
          LudoPlayer(colors[i], i == 0 ? PlayerType.human : PlayerType.ai),
        );
      }
    } else if (_numPlayers == 2) {
      // 2 humans, opposite corners
      final otherColor = ludoOppositeColor(_humanColor);
    ...
    _finishOrder = [];
    _burstGridPos = null;
    _celebrateHome = false;
    _celebrateHomeColor = null;
  ...
  void _rollDice() {
    ...
    _diceAnimCtrl!.forward().then((_) {
      ...
      if (movable.isEmpty) {
        _status =
            '${_colorName(_currentPlayer.color)} rolled $_diceValue — no moves';
        setState(() {});
        Future.delayed(const Duration(milliseconds: 800), () {
          if (!mounted || generation != _gameGeneration || _gameOver) return;
          if (_diceValue == 6) {
            _diceRolled = false;
            _status =
                '${_colorName(_currentPlayer.color)} rolled 6 — roll again';
            setState(() {});
            if (_currentPlayer.type == PlayerType.ai) _scheduleAI();
          } else {
            _nextTurn();
          }
        });
        ...
        _status =
            '${_colorName(_currentPlayer.color)} rolled $_diceValue — pick a piece';
        setState(() {});
  ...
  List<LudoPiece> _getMovablePieces() {
    ...
    for (final piece in _currentPlayer.pieces) {
      ...
      if (piece.position == -1) {
        ...
        if (canAdvanceLudoPiece(piece.position, _diceValue)) {
          movable.add(piece);
        }
      }
    }
  ...
  void _movePiece(LudoPiece piece) {
    ...
    if (piece.position == -1) {
      ...
      if (piece.position == ludoFinishPosition) {
        // Reached home!
        ...
        Future.delayed(const Duration(milliseconds: 900), () {
          if (mounted && generation == _gameGeneration) {
            setState(() => _celebrateHome = false);
          }
        });
        ...
        if (_currentPlayer.allFinished) {
          ...
          if (_finishOrder.length >= _players.length - 1) {
            ...
            Future.delayed(const Duration(milliseconds: 300), () {
              if (mounted && generation == _gameGeneration && _gameOver) {
                _showWinDialog();
              }
            });
            return;
  ...
  bool _checkCapture(LudoPiece piece) {
    ...
    for (final player in _players) {
      ...
      for (final other in player.pieces) {
        if (other.isFinished || other.position < 0 || other.position > 51) {
          continue;
        }
        if (_absolutePosition(other) == absPos) {
          ...
          HapticFeedback.heavyImpact();
          _status =
              '${_colorName(piece.color)} captures ${_colorName(other.color)}!';
  ...
  void _aiPickPiece(List<LudoPiece> movable) {
    ...
    for (final piece in movable) {
      ...
      if (piece.position == -1) {
        ...
        final inBase = _currentPlayer.pieces
            .where((p) => p.position == -1 && !p.isFinished)
            .length;
        final onBoard = _currentPlayer.pieces
            .where((p) => p.position >= 0 && !p.isFinished)
            .length;
        ...
        if (newPos == ludoFinishPosition) {
          ...
          if (canCapture) {
            score = captureAdvancedPiece
                ? 400
                : 250; // Capture, especially advanced opponents
          }
      ...
      child: TweenAnimationBuilder<double>(
        ...
        builder: (_, v, __) => Opacity(
          ...
          child: SizedBox(
            ...
            height: cell * 4,
            child: CustomPaint(
              painter: _CelebrationPainter(progress: v, color: color),
            ),
          ),
        ),
      ),
                    ...
                    .map(
                      (c) => Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          boxShadow: [
                            BoxShadow(
                              color: _ludoColors[c]!.withValues(alpha: 0.4),
                              blurRadius: 8,
                            ),
                          ],
                        ),
      ...
      body: SafeArea(
        child: LayoutBuilder(
          builder: (context, constraints) {
            final boardSize = min(
              min(constraints.maxWidth - 24, constraints.maxHeight - 200),
              700.0,
            );
              ...
              children: [
                ...
                Padding(
                  ...
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                    children: _players
                        .map((p) => Flexible(child: _playerChip(p)))
                        .toList(),
                  ),
                ),
                      ...
                      child: CustomPaint(
                        ...
                        child: Stack(
                          children: [
                            ...
                            // Draw the active player's pieces last so their
                            // touch targets always remain above opponents.
                            for (final piece in _currentPlayer.pieces)
                              if (!piece.isFinished)
                                _buildPieceOnBoard(piece, boardSize),
                ...
                Row(
                  ...
                  children: [
                    ...
                    if (!_diceRolled &&
                        !_gameOver &&
                        _currentPlayer.type == PlayerType.human)
                      ElevatedButton(
  ...
  List<LudoPiece> _piecesAtSamePosition(LudoPiece piece) {
    final pieces = <LudoPiece>[];
    final absPos = piece.position >= 0 && piece.position <= 51
        ? _absolutePosition(piece)
        : -999;
    for (final player in _players) {
      for (final other in player.pieces) {
    ...
    // Put the active color later in the group, matching its higher paint order.
    pieces.sort((a, b) {
      final aActive = a.color == _currentPlayer.color ? 1 : 0;
      final bActive = b.color == _currentPlayer.color ? 1 : 0;
      final activeOrder = aActive.compareTo(bActive);
      if (activeOrder != 0) return activeOrder;
      final colorOrder = a.color.index.compareTo(b.color.index);
      return colorOrder != 0 ? colorOrder : a.index.compareTo(b.index);
    });
    return pieces;
  }
  ...
  Widget _buildPieceOnBoard(LudoPiece piece, double boardSize) {
    ...
    final isMovable =
        _diceRolled &&
        piece.color == _currentPlayer.color &&
        _getMovablePieces().contains(piece);
    final stack = _piecesAtSamePosition(piece);
    final stackCount = stack.length;
    final stackIndex = stack.indexOf(piece);
    final stackOffset = ludoStackOffset(stackIndex, stackCount, cellSize);
    final pieceSize = cellSize * (stackCount > 2 ? 0.48 : 0.58);

    return Positioned(
      left: pos.dx - pieceSize / 2 + stackOffset.dx,
      top: pos.dy - pieceSize / 2 + stackOffset.dy,
          ...
          children: [
            AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: [
                  BoxShadow(
                    color: color.withValues(alpha: isMovable ? 0.6 : 0.3),
...
class _LudoBoardPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    _drawColoredCell(
      canvas,
      cell,
      _trackCoordinates[13],
      _ludoColors[LudoColor.green]!,
    );
    _drawColoredCell(
      canvas,
      cell,
      _trackCoordinates[26],
      _ludoColors[LudoColor.yellow]!,
    );
    _drawColoredCell(
      canvas,
      cell,
      _trackCoordinates[39],
      _ludoColors[LudoColor.blue]!,
    );
  }
...
class _DiceFacePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    switch (value) {
      case 1:
        positions.addAll([Offset(cx, cy)]);
      case 2:
        positions.addAll([Offset(cx - d, cy - d), Offset(cx + d, cy + d)]);
      case 3:
        positions.addAll([
          Offset(cx - d, cy + d),
          Offset(cx, cy),
...
class _BurstPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final particlePaint = Paint()
      ..color = Color.fromRGBO(255, 165, 0, (1.0 - progress) * 0.8);
    for (int i = 0; i < 8; i++) {
...
class _CelebrationPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(
      center,
      maxRadius * 0.3 * (1.0 + progress * 0.5),
